...
class VentanaPrincipal(tk.Tk):
    def __init__(self):
        ...
        self.after(600, lambda: tal_vez_iniciar_tutorial_general(self))
    ...
    def _crear_sidebar(self, parent):
        ...
        self._btn_ayuda = ttk.Button(
            sidebar,
            text="❓  Ayuda y tutorial",
            style="Secundario.TButton",
            command=lambda: abrir_centro_ayuda(self)
        )
        self._btn_ayuda.pack(fill="x", padx=8, pady=(0, 4))
    ...
    # ── API pública usada por el sistema de tutorial ────────────────
    # (tutorial.py y tutorial_practice.py se apoyan en estos métodos
    # en vez de tocar directamente los atributos internos del sidebar
    # y la navegación, para no duplicar esa lógica.)
    def navegar(self, clave: str):
        """Navega a un módulo por su clave (igual que hace el sidebar)."""
        self._navegar(clave)

    def obtener_vista(self, clave: str):
        """Devuelve la instancia de la vista de ese módulo, o None si el
        rol actual no tiene acceso."""
        return self._vistas.get(clave)

    def vista_activa(self):
        """Devuelve la vista actualmente visible."""
        return self._vistas.get(self._clave_activa)

    def boton_menu(self, clave: str):
        """Devuelve el botón del sidebar de ese módulo, o None."""
        return self._botones_menu.get(clave)

    def widget_sidebar(self):
        """Devuelve el Frame del menú lateral completo (para resaltarlo)."""
        return self._sidebar

    def boton_ayuda(self):
        """Devuelve el botón «❓ Ayuda y tutorial» del sidebar."""
